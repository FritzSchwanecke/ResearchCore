...
namespace ResearchCore.Helper.Functions
{
    ...
    {
        private static readonly double norm_cdf_asymptotic_expansion_first_threshold = -10.0;

        private static readonly double norm_cdf_asymptotic_expansion_second_threshold =
            -1.0 / Math.Sqrt(double.MinValue);
            ...
            if (z <= norm_cdf_asymptotic_expansion_first_threshold)
            {
                ...
                if (z >= norm_cdf_asymptotic_expansion_second_threshold)
                {
                    ...
                    do
                    {
                        ...
                        ++i;
                        a = Math.Abs(a);
                    } while (lasta > a && a >= Math.Abs(sum * double.MinValue));
                }

                return -NormPdf(z) * sum / z;
            }

            return 0.5 * Erfc.ErfcCody(-z * OneOverSqrtTwo);
        }

        public static double InverseNormCdf(double u)
        {
            //
        ...
        {
            return OneOverSqrtTwoPi * Math.Exp(-.5 * x * x);
        }
    }
}